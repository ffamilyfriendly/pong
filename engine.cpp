...
void calcCol() {
    //check if wall
    if(ball->y_pos >= 1) ball->y_speed = ball->y_speed-ball->y_speed*2;
    ...
    if(ball->y_pos <= p2->y_pos+p2->height && ball->y_pos > p2->y_pos && ball->x_pos >= 1-p2->width) ball->x_speed = ball->x_speed-ball->x_speed*2;
